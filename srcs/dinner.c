...
void *dinner_simulation(void *data)
{
	...
	set_long(&philo->philo_mutex, &philo->last_meal_time ,gettime(MILISECONDS));
	increase_long(&philo->table->table_mutex,&philo->table->nbr_of_threads_running);
	godricks_hat(philo);
	while (!simulation_finish(philo->table))
	{
		if (philo->is_full)//get bool maybe ? 
			break;
		eat(philo);
...
static void *darwinism(void * data)
{
	...
	set_long(&philo->table->table_mutex, &philo->last_meal_time,gettime(MILISECONDS));
	increase_long(&philo->table->table_mutex,&philo->table->nbr_of_threads_running);
	write_status(FIRST_FORK,philo);
	while(!simulation_finish(philo->table))
		usleep(200);
	...
	if (table->meals_limit == 0)
		return;
		 ...
		 while (++i < table->num_of_philos)
			safe_thread_handle(&table->philos[i].thread_id, dinner_simulation, CREATE, &table->philos[i]);
